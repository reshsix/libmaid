...
mixcolumn_e(u8 *block)
{
    ...
    for (u8 i = 0; i < 4; i++)
    {
        ...
        memcpy(t, col, 4);
        col[0] = gmul2[t[0]] ^ gmul3[t[1]] ^       t[2]  ^       t[3] ;
        col[1] =       t[0]  ^ gmul2[t[1]] ^ gmul3[t[2]] ^       t[3] ;
        col[2] =       t[0]  ^       t[1]  ^ gmul2[t[2]] ^ gmul3[t[3]];
...
mixcolumn_d(u8 *block)
{
    ...
    for (u8 i = 0; i < 4; i++)
    {
        ...
        /* Matrix multiplication in the Galois field */
        memcpy(t, col, 4);
        col[0] = gmul14[t[0]] ^ gmul11[t[1]] ^ gmul13[t[2]] ^  gmul9[t[3]];
        col[1] =  gmul9[t[0]] ^ gmul14[t[1]] ^ gmul11[t[2]] ^ gmul13[t[3]];
...
aes_decrypt(void *ctx, u8 *block)
{
    if (ctx && block)
    {
        ...
        for (u8 i = 1; i < aes->nr; i++)
        {
            shiftrow_d(block);
            bytesub_d(block);
            addroundkey(aes->ctx, block, aes->nr - i);
            mixcolumn_d(block);
        }
        shiftrow_d(block);
        bytesub_d(block);
