...
static void
sha2_rounds(u8 version, void *h, const u8 *message)
{
    switch (version)
    {
        case SHA_224:;
...
static void
sha2_output(u8 version, void *h, u8 *tag)
{
    u8 length = 0;
    bool bits64 = false;
    bool trunc = false;
...
sha2_new(u8 version)
{
    struct sha2 *s2 = calloc(1, sizeof(struct sha2));

    if (s2)
    {
        s2->version = version;
        sha2_init(s2->version, &(s2->h));
...
static void
sha2_update(void *ctx, u8 *buffer, size_t size)
{
    if (ctx)
    {
        struct sha2 *s2 = ctx;
        ...
        if (size >= limit1)
            sha2_rounds(s2->version, &(s2->h), buffer);
        else
        {
            u8 buffer2[128] = {0};
            s2->last = true;
            ...
            if (size >= limit2)
            {
                sha2_rounds(s2->version, &(s2->h), buffer2);
...
static void
sha2_digest(void *ctx, u8 *output)
{
    if (ctx)
    {
        struct sha2 *s2 = ctx;
        ...
        if (!(s2->last))
            sha2_update(ctx, NULL, 0);

        sha2_output(s2->version, &(s2->h), output);
    }
}
