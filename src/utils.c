...
extern size_t
maid_cb_splitter(void *ctx, u8 *data, size_t bytes)
{
    size_t ret = 0;

    if (ctx && data)
    {
        struct maid_cb_split *split = ctx;
        if (split->read && split->read->f)
            ret = split->read->f(split->read->ctx, data, bytes);

        if (split->write && split->write->f)
            split->write->f(split->write->ctx, data, ret);
    }
...
extern size_t
maid_cb_saver(void *ctx, u8 *data, size_t bytes)
{
    size_t ret = 0;

    if (ctx && data)
    {
        struct maid_cb_save *save = ctx;
        if (save->read && save->read->f)
            ret = save->read->f(save->read->ctx, data, bytes);

        if (save->buffer)
        {
            if (save->buffer_s > bytes)
            {
                memcpy(save->buffer, data, bytes);
                memset(&(save->buffer[bytes]), '\0', save->buffer_s - bytes);
            }
            else
                memcpy(save->buffer, data, save->buffer_s);
        }
    }
...
extern size_t
maid_cb_loader(void *ctx, u8 *data, size_t bytes)
{
    size_t ret = 0;

    if (ctx && data)
    {
        struct maid_cb_load *load = ctx;
        if (load->read && load->read->f)
            ret = load->read->f(load->read->ctx, data, bytes);

        if (load->saved)
        {
            size_t buf_s = load->saved->buffer_s;
            if (buf_s < bytes)
            {
                memcpy(data, load->saved->buffer, bytes);
                memset(&(data[buf_s]), '\0', bytes - buf_s);
            }
            else
                memcpy(data, load->saved->buffer, bytes);
        }
